        ...
        if(isset($_FILES['image'])){
          ...
          if(in_array($img_ext,$extention) === true){
            ...
            if (move_uploaded_file($tmp_name,"img/".$new_img_name)){
              ...
              $stmt2->bindValue(':lname', $fname);
              $stmt2->bindValue(':email', $lname);
              $stmt2->bindValue(':password', $email);
              $stmt2->bindValue(':new_img_name', $password);
              $stmt2->bindValue(':status', $new_img_name);
              // executeでクエリを実行
              $stmt2->execute();
